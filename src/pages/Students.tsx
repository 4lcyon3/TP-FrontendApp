...
import GroupPredictModal from "@/utils/GroupPredictModal";
...
export default function StudentsPage() {
  ...
  // Cálculo de estado + Ordenamiento alfabético por Nombre
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter !== "all") result = result.filter(s => s.calculatedStatus === statusFilter);
  ...
  const top5LowPerformers = useMemo(() => {
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (top5Metric === 'avg') { valA = (a.score_total || 0) / countA; valB = (b.score_total || 0) / countB; }
      else if (top5Metric === 'c1') { valA = (a.persistente_total || 0) / countA; valB = (b.persistente_total || 0) / countB; }
      else if (top5Metric === 'c2') { valA = (a.competente_total || 0) / countA; valB = (b.competente_total || 0) / countB; }
      else if (top5Metric === 'c3') { valA = (a.observador_total || 0) / countA; valB = (b.observador_total || 0) / countB; }
      return valA - valB;
    });
    ...
    <div className="space-y-6 animate-fade-in pb-10 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Lista de alumnos a tu cargo y análisis de comprensión lectora.</p>
        </div>
      ...
      {/* Gráficos */}
      <div className="grid grid-cols-1 [@media(min-width:900px)]:grid-cols-3 gap-6">
        {/* Card 1 */}
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col">
          <div className="flex justify-between items-center mb-2">
            ...
            <select value={top5Metric} onChange={(e: ChangeEvent<HTMLSelectElement>) => setTop5Metric(e.target.value as "avg" | "c1" | "c2" | "c3")} className="text-xs border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded px-2 py-1 focus:ring-1 focus:ring-indigo-500 outline-none cursor-pointer">
              ...
              <tbody>
                {top5LowPerformers.map((student, idx) => (
                  <tr key={idx} className="border-b last:border-0 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                    <td className="py-2 text-slate-700 dark:text-slate-300 truncate max-w-[120px]" title={student.name}>{idx + 1}. {student.name}</td>
                    <td className="py-2 text-right font-bold" style={{ color: student.color }}>{student.value}</td>
                  </tr>
                ))}
                {top5LowPerformers.length === 0 && (<tr><td colSpan={2} className="text-center py-4 text-slate-400">Sin datos</td></tr>)}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col justify-between relative overflow-hidden">
          <div className="relative z-10">
            ...
            <select value={selectedStudentId || ""} onChange={(e) => setSelectedStudentId(Number(e.target.value))} className="w-full text-sm border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded px-3 py-2 mb-4 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer">
              {filtered.length === 0 && <option value="">Sin alumnos</option>}
              {filtered.map(s => (<option key={s.id} value={s.id}>{s.first_name} {s.last_name}</option>))}
            </select>
            {selectedStudentData ? (
              <div className="flex flex-col items-center justify-center py-2">
                ...
                <div className="text-6xl font-extrabold tracking-tighter transition-colors duration-300" style={{ color: selectedStudentData.average < 3.45 ? '#ef4444' : selectedStudentData.average < 6.5 ? '#f59e0b' : '#10b981' }}>
                  {selectedStudentData.average.toFixed(2)}
                </div>
                <div className="mt-3 flex items-center gap-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold text-white shadow-sm ${selectedStudentData.average < 3.45 ? 'bg-rose-500' : selectedStudentData.average < 6.5 ? 'bg-amber-500' : 'bg-emerald-500'}`}>
                    {selectedStudentData.calculatedStatus}
                  </span>
                  <span className="text-xs text-slate-400">({selectedStudentData.cant_evaluaciones} eval.)</span>
                </div>
              </div>
            ) : (
              <div className="flex items-center justify-center h-32 text-slate-400 text-sm">Seleccione un alumno</div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
          <button onClick={() => { setShowFlowDiagram(!showFlowDiagram); setFlowCollapsed(false); }} className="w-full flex items-center justify-between p-5 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
          ...
          {showFlowDiagram && !flowCollapsed && (
            <div className="p-5 pt-0 animate-slide-down">
              <div className="grid grid-cols-5 gap-3 text-center">
                {[{ icon: "🎮", title: "1. Kahoot!", desc: "Pregunta gamificada" }, { icon: "📝", title: "2. ClassDojo", desc: "Registrar C1, C2, C3" }, { icon: "➕", title: "3. Extras", desc: "Puntos adicionales" }, { icon: "📤", title: "4. Exportar", desc: "Bajar CSV" }, { icon: "📊", title: "5. App", desc: "Ver predicciones" }].map((step, idx) => (
              ...
              <button onClick={() => setFlowCollapsed(true)} className="mt-3 text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 flex items-center gap-1 mx-auto"><ChevronUp size={12} /> Colapsar</button>
            </div>
          )}
          {showFlowDiagram && flowCollapsed && (<div className="px-5 pb-4"><button onClick={() => setFlowCollapsed(false)} className="text-xs text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 flex items-center gap-1"><ChevronDown size={12} /> Expandir flujo</button></div>)}
        </div>

        {/* Leyenda (Minimizada por defecto) */}
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
          <button onClick={() => setStatsCollapsed(!statsCollapsed)} className="w-full flex items-center justify-between p-5 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
            <div className="flex items-center gap-3">
            ...
            <div className="p-5 pt-0 space-y-4 animate-slide-down">
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-100 dark:border-blue-800">
                  <div className="flex items-center gap-2 mb-1"><Award className="text-blue-600 dark:text-blue-400" size={16} /><h4 className="font-bold text-blue-800 dark:text-blue-300 text-sm">Persistente (C1)</h4></div>
                ...
                <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg border border-purple-100 dark:border-purple-800">
                  <div className="flex items-center gap-2 mb-1"><BrainCircuit className="text-purple-600 dark:text-purple-400" size={16} /><h4 className="font-bold text-purple-800 dark:text-purple-300 text-sm">Competente (C2)</h4></div>
                ...
                <div className="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-lg border border-orange-100 dark:border-orange-800">
                  <div className="flex items-center gap-2 mb-1"><Info className="text-orange-600 dark:text-orange-400" size={16} /><h4 className="font-bold text-orange-800 dark:text-orange-300 text-sm">Observador (C3)</h4></div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por nombre..." value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all" />
        </div>
        <div className="flex items-center gap-2">
          <Filter size={18} className="text-slate-500" />
          <select value={statusFilter} onChange={(e: ChangeEvent<HTMLSelectElement>) => { setStatusFilter(e.target.value as "all" | "Aprobado" | "En Riesgo" | "Desaprobado"); setPage(1); }} className="py-2 px-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={sectionFilter} onChange={(e: ChangeEvent<HTMLSelectElement>) => { setSectionFilter(e.target.value); setPage(1); }} className="py-2 px-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer">
            <option value="all">Todas las secciones</option>
            {availableSections.map(section => (<option key={section} value={section}>{section}</option>))}
          </select>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => setUploadOpen(true)} className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition shadow text-sm"><Upload className="w-4 h-4" /> Subir reporte</Button>
          <Button onClick={() => setGroupPredictOpen(true)} disabled={filtered.length === 0} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg transition shadow text-sm disabled:opacity-50 disabled:cursor-not-allowed"><Users size={16} /> Predecir Grupo</Button>
          <Button onClick={() => exportStudentsExcel(filtered)} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg transition shadow text-sm"><FileSpreadsheet size={16} /> Exportar Excel</Button>
        </div>
        <div className="text-sm text-slate-500 dark:text-slate-400 font-medium whitespace-nowrap flex items-center">{total} alumnos</div>
      </div>
        ...
        <table className="min-w-full">
          <thead className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
            <tr>
              ...
              <th className="px-4 py-3 text-center text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase">Persistente (C1)<Tooltip text="Obtiene información del texto escrito." /></th>
              <th className="px-4 py-3 text-center text-xs font-semibold text-purple-600 dark:text-purple-400 uppercase">Competente (C2)<Tooltip text="Infiere e interpreta información." /></th>
              <th className="px-4 py-3 text-center text-xs font-semibold text-orange-600 dark:text-orange-400 uppercase">Observador (C3)<Tooltip text="Reflexiona y evalúa el texto." /></th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {paginated.map((s, idx) => {
              ...
              return (
                <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  ...
                  {/* CAMBIO: Botones en fila (móvil) con badge abajo */}
                  <td className="px-4 py-3 text-sm">
                    <div className="flex flex-row items-center gap-2 flex-wrap">
                      <button className="p-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition shadow-sm" onClick={() => setEditStudent(s)} title="Editar">
                        <Edit size={16} />
                      </button>
                      <button className="p-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition shadow-sm" onClick={() => setConfirmDelete(s)} title="Eliminar">
                        <Trash2 size={16} />
                      </button>
                      <button className="p-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition shadow-sm" onClick={() => { setSelectedStudent(s.id); setPredictOpen(true); }} title="Predecir">
                        <Cpu size={16}/>
                      </button>
                      {count > 0 && (
                        <span className={`ml-2 px-2 py-1 rounded-full text-[10px] font-bold whitespace-nowrap ${statusColor}`}>
                          {statusText}
                        </span>
                      )}
                    </div>
            ...
            {paginated.length === 0 && (<tr><td colSpan={8} className="py-8 text-center text-slate-500 dark:text-slate-400">No hay estudiantes que coincidan con los filtros.</td></tr>)}
          </tbody>
        </table>
      </div>
      ...
      {groupPredictOpen && (<GroupPredictModal isOpen={groupPredictOpen} onClose={() => setGroupPredictOpen(false)} students={filtered} filterDescription={`${statusFilter !== 'all' ? statusFilter : 'Todos'} ${sectionFilter !== 'all' ? '- ' + sectionFilter : ''}`} />)}
    </div>
  );
}